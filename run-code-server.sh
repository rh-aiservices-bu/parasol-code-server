...
# Initialize Continue configuration if not present
# Check if the config file already exists
if [ ! -f /opt/app-root/src/.continue/config.json ]; then
  # Create the directory if it doesn't exist
  mkdir -p /opt/app-root/src/.continue

  # Copy the config file
  cp /opt/app-root/bin/utils/config.json /opt/app-root/src/.continue/config.json

  # Replace placeholders with environment variable values
  sed -i "s|MODEL_ENDPOINT_URL|${MODEL_ENDPOINT_URL}|g" /opt/app-root/src/.continue/config.json
  sed -i "s|API_KEY|${API_KEY}|g" /opt/app-root/src/.continue/config.json
  sed -i "s|MODEL_NAME|${MODEL_NAME}|g" /opt/app-root/src/.continue/config.json
fi
